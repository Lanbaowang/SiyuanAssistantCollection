...
//an array includes all unhandled blocks(info)
export const 创建切片 = async (originalBlocksArray, 分片尺寸) => {
    const 切片数组 = [];
    for (let i = 0; i < Math.ceil(originalBlocksArray.length / 分片尺寸); i++) {
        const 切片 = originalBlocksArray.slice(i * 分片尺寸, (i + 1) * 分片尺寸);
        切片数组.push(切片);
    }
    return 切片数组
}
/**
  * 批处理索引切片
  * @param {Array} originalBlocksArray - 需要处理的originalBlocksArray, an array includes all unhandled blocks(info)
  * @returns {Object} - 返回一个对象，包含处理的块数量和总处理时长
  * 
  * 注意：这个函数假设originalBlocksArray是一个非空数组，如果不是，函数可能会抛出错误。
  * 另外，这个函数使用了多个worker并行处理切片，如果worker处理切片时出错，可能会导致整个函数失败。
  * 最后，这个函数的处理时间可能会很长，如果处理时间过长，可能会影响用户的体验。
  */
export const 批处理索引切片 = async (originalBlocksArray) => {
    if (!Array.isArray(originalBlocksArray) || originalBlocksArray.length === 0) {
        throw new Error('originalBlocksArray必须是一个非空数组');
    }
    let box = originalBlocksArray[0].box
    let noteBookInfo = await 根据笔记本ID获取笔记本(box)
    logger.blockIndexlog(`开始处理笔记本:${noteBookInfo.name}${originalBlocksArray[0].box},总计${originalBlocksArray.length}个块`);
    let 处理开始时间 = performance.now();
    //这里给出设置
    const 切片数组 = await 创建切片(originalBlocksArray, plugin.configurer.get('向量工具设置', '块索引分片大小').$value);
    const numWorkers = navigator.hardwareConcurrency || 8;
    await 处理切片数组(切片数组, numWorkers, originalBlocksArray, 处理开始时间);
    let 处理时长 = (performance.now() - 处理开始时间) / 1000;
    logger.blockIndexlog(`笔记本:${originalBlocksArray[0].box}处理时长为${处理时长},总计块${originalBlocksArray.length},单块处理时长约${处理时长 / originalBlocksArray.length || 0}秒`);
    await 清理索引();
    return { 块数量: originalBlocksArray.length, 处理时长: 处理时长 };
}
const 处理切片数组 = async (切片数组, numWorkers, originalBlocksArray, 处理开始时间) => {
    let 已处理数量 = 0;
    for (let i = 0; i < 切片数组.length; i += numWorkers) {
        const 子切片数组 = 切片数组.slice(i, i + numWorkers);
        await 处理子切片数组(子切片数组);
        已处理数量 += 子切片数组.length;
        打印处理进度(originalBlocksArray, 已处理数量, 处理开始时间);
    }
    return 已处理数量;
}
...
export const 获取全块数组 = async () => {
    ...
    console.log('blockIndex: sql: ' + 全块数组获取语句)
...
export const 处理子切片数组 = async (子切片数组) => {
    try {
        await Promise.all(子切片数组.map(子切片 => 处理子切片(子切片, plugin.configurer.get('向量工具设置', '最大句子长度').$value)));
    } catch (error) {
...
export const 打印处理进度 = (originalBlocksArray, 已处理数量, 处理开始时间) => {
    const 处理时长 = (performance.now() - 处理开始时间) / 1000;
    const 切片大小 = plugin.configurer.get('向量工具设置', '块索引分片大小').$value
    const 单块处理时长 = 处理时长 / 已处理数量 / 切片大小;
    const 剩余块数量 = originalBlocksArray.length - 已处理数量 * 切片大小;
    logger.blockIndexlog(`笔记本:${originalBlocksArray[0].box}已处理${已处理数量 * 切片大小}个块,剩余${剩余块数量},单块处理时长约${单块处理时长}秒,请耐心等候,你可以继续记录,不受影响`);
}
export const 处理所有笔记本数据 = async (boxMap) => {
    let 总块数量 = 0, 总处理时长 = 0
    for (const blocksArray of boxMap.values()) { //boxMap = {"box_id": blocksArray}
        let result = await 批处理索引切片(blocksArray)
        总块数量 += result.块数量
        总处理时长 += result.处理时长
    }
    return { 总块数量, 总处理时长 };
