...
import { 将向量单位化, getWeightedMeanVectors } from './vector.js';
...
export async function 提取向量(text, 最大句子长度) {
    ...
    try {
        ...
        for (let 句子 of 句子组) {
            if (句子) {
                let output = await feature_extractor(句子, { pooling: 'mean', normalize: true });
                句子向量组.push(Array.from(output.data));
            }
        }

        if (句子向量组.length > 0) {
            return getWeightedMeanVectors(句子向量组, 句子长度比例组);
        } else {
            return [];
        }
    } catch (e) {
